...
//	vnclog.Print(0, _T("IsShortcutFolder\n"));
	...
	sprintf(szGUIDir, "%s%s%s", rfbDirPrefix, "내 문서", rfbDirSuffix);
	if (!_strnicmp(szPath, szGUIDir, strlen(szGUIDir)))
		return true;

	sprintf(szGUIDir, "%s%s%s", rfbDirPrefix, "바탕화면", rfbDirSuffix);
	if (!_strnicmp(szPath, szGUIDir, strlen(szGUIDir)))
		return true;

	sprintf(szGUIDir, "%s%s%s", rfbDirPrefix, "즐겨찾기", rfbDirSuffix);
	if (!_strnicmp(szPath, szGUIDir, strlen(szGUIDir)))
		return true;
...
//	vnclog.Print(0, _T("ListRemoteDrives\n"));
	...
	while (nIndex < nLen - 3)
	{
		...
		{
		case 'l':
			sprintf(szType, "%s", "로컬 디스크");
			break;
		case 'f':
			sprintf(szType, "%s", "이동식");
			break;
		case 'c':
			sprintf(szType, "%s", "CD-ROM");
			break;
		case 'n':
			sprintf(szType, "%s", "네트워크");
			break;
		default:
			sprintf(szType, "%s", "알수없음");
			break;
		}
	...
	*strrchr(szDestPath, '\\') = '\0'; // We don't handle UNCs for now
	...
	if (fErr)
	{
		if (!fErrNoFileName)
			...
			sprintf(szStatus, " %s < %s > %s",sz_H14, "잘못된 원격지 파일명", sz_H13); 
	...
	case WM_INITDIALOG:
		{
			...
			_snprintf(szTitle, lTitleBufSize-1," %s < %s>  -  runSupport", sz_H35,szRemoteName);
...
FILETIME FileTransfer::GetFileTimeFromString(char* szFileSystemTime)
{
	...
	{
		sscanf(szFileSystemTime,"%2d/%2d/%4d %2d:%2d",
				&m,
				&d,
				&y,
				&h,
